...
if __name__ == "__main__":
    ...
    def sortby(element):
        if isinstance(element['func_data'], dict):
            return element['func_data']['initTime_ms']
        else:
            return 0
    res_list = sorted(res_list, key=sortby)
    stats_by_container = {}
    for func_stats in res_list:
        if isinstance(func_stats['func_data'], dict):
    ...
    for k, container_stats in stats_by_container.items():
        if k != "ERROR":
            stats_by_container[k] = sorted(
                container_stats, key=lambda i: i['func_data']['initTime_ms'])
    ...
    # gnt.set(title=f"GCP - {n_func} requests")

    # Setting Y-axis limits
    gnt.set_ylim(0, n_func*10)

    # Setting X-axis limits
    gnt.set_xlim(-500, max_time+100)
    ...
    for container_id, container_stats in stats_by_container.items():
        ...
        for invocation_stats in container_stats:
            ...
            if container_id != "ERROR":
                gnt.broken_barh([(invocation_stats['func_data']['initTime_ms']
                                  - min_time,
                                  invocation_stats['func_data']['duration_ms'])],
                                (invocation_i, 9), facecolors=color)
            else:
                gnt.broken_barh([(invocation_stats['pyend_ms'] - min_time - 1500,
                                  1000)],
                                (invocation_i, 9), facecolors='k',
                                hatch='/', edgecolor='r')
